...
import { TbAccessPoint, TbSteeringWheel } from "react-icons/tb";
...
import SearchByCarNumber from "./SearchByCarNumber";
...
export default function AddCar() {
    ...
    const { contactName,customerSet } = useContext(ContactContext);
    ...
    async function GetVichel() {
        ...
        setLoading(true);
        let wichOne = 0;
        const response = await fetch(`https://data.gov.il/api/3/action/datastore_search?resource_id=053cea08-09bc-40ec-8f7a-156f0677aff3&q=${carNumber.replace(/-/g, '')}`);
        const movies = await response.json();
        const ress = movies.result.records;
        if (ress[0]) {
            setEndDateCar(ress[0].tokef_dt.slice(0, 10));
            if(ress[0].tokef_dt){wichOne++;}
        }
        const response1 = await fetch(`https://data.gov.il/api/3/action/datastore_search?resource_id=cd3acc5c-03c3-4c89-9c54-d40f93c0d790&q=${carNumber.replace(/-/g, '')}`);
        const movies1 = await response1.json();
        const ress1 = movies1.result.records;
        if (ress1[0]) {
            setEndDateCar(ress1[0].tokef_dt.slice(0, 10));
            if(ress1[0].tokef_dt){wichOne++;}
        }
        const response2 = await fetch(`https://data.gov.il/api/3/action/datastore_search?resource_id=58dc4654-16b1-42ed-8170-98fadec153ea&q=${carNumber.replace(/-/g, '')}`);
        const movies2 = await response2.json();
        const ress2 = movies2.result.records;
        if (ress2[0]) {
            setEndDateCar(ress2[0].tokef_dt.slice(0, 10));
            if(ress2[0].tokef_dt){wichOne++;}
        }
        setLoading(false);
        if (wichOne == 0) {return setErrorCarNumber('מספר הרכב שהוזן לא נמצא !');; }
        setErrorCarNumber('');
    }
    ...
    const [showModalSearchCarByNumber,setShowModalSearchByCarNumber] = useState(false);
    ...
    const [reshionMovel, setReshionMovel] = useState('');
    const [errorReshionMovel, setErrorReshionMovel] = useState('');
    const [customerImage, setCustomerImage] = useState('');
    const [errorCustomerImage, setErrorCustomerImage] = useState('');
    ...
    const reshionMovelRef = useRef();
    const CustomerImageRef = useRef();
    ...
    const resetAllErrors = () => {
        ...
        setErrorHazmatDDate('');
        setErrorReshionMovel('');
        setErrorCustomerImage('');
    }
    ...
    // רישיון מוביל
    const reshionMovelFile = useRef();
    const [reshionMovelDateFile, setReshionMovelDateFile] = useState(null);

    // תמונת לקוח
    const customerImageFile = useRef();
    const [customerImageDateFile, setcustomerImageDateFile] = useState(null);
    ...
    const ResetAllFilesCustomer = () => {
        setReshionMovelDateFile('');
        setcustomerImageDateFile('');
    }
    ...
    const AddCustomer = async () => {
        ...
        if (!customer) {
            ...
            if (!reshionMovel) {
                setErrorReshionMovel('לא הוזנו נתונים !');
            }
            if (isNaN(Date.parse(reshionMovel))) {
                numErrors++;
                setErrorReshionMovel('התאריך חייב להיות DD/MM/YYYY | DD-MM-YYYY | DD.MM.YYYY !');
            }
            if (numErrors != 0) {
                return;
            }
        ...
        if (reshionMovelDateFile) {
            const storageRefReshionMovel = ref(storagee, `${customer ? customer.customer_phone : number}/ReshionMovel`);
            const uploadTaskReshionMovel = uploadBytesResumable(storageRefReshionMovel, reshionMovelDateFile);
        }
        if (customerImageDateFile) {
            const storageRefImageCus = ref(storagee, `${customer ? customer.customer_phone : number}/CustomerImage`);
            const uploadTaskImageCus = uploadBytesResumable(storageRefImageCus, customerImageDateFile);
        }
        ResetAllFilesCustomer();
        resetAllCustomer();
    ...
    const AddCar = async () => {
        ...
        !driver && isWithDriver && await addDoc(collection(MohamadFireStore, "Driver"), NewDataDriver) && await updateDoc(doc(MohamadFireStore, "numbers", 'drivers'), { number: DriversNum + 1 });
        if (car) {
            ...
            const NewDataCar = {
                Driver_id: isWithDriver ? (driver ? driver.driver_id : currectDriverId()) : null,
                car_id: currectCarId(),
                car_num: carNumber,
                ...
                shockabsorbers: shockabsorbers ? shockabsorbers : null,
                tachograph: tachograph === 'כן' ? true : false,
                tachographDate: tachograph === 'כן' ? tachographDate : null,
                winterreview: winterreview,
                insuranceCompany: insuranceCompany,
                monthlyReview: null,
                zmenotRekhev: isWithDriver ? false : true
            }
            const NewDataRef = await addDoc(collection(MohamadFireStore, "car"), NewDataCar);
            const docSnap = await getDoc(NewDataRef);
            setShowFormCar(docSnap.exists() ? { ...docSnap.data(), id: docSnap.id } : null);
        ...
        setIsWithDriver(true);
        setShowCustomerModdal(true);
    }
    ...
    const sortCarNumber = (value1) => {
        let value = value1.replace(/\D/g, '');
        if (value.length > 8) {
            value = value.substring(0, 8);
        }
        if (value.length >= 1 && value.length <= 5) {
            value = value.replace(/(\d{2})(\d{1,3})?/, "$1-$2").trim();
        } 
        else if (value.length >= 6 && value.length <= 7) {
            value = value.replace(/(\d{2})(\d{3})(\d{1,2})?/, "$1-$2-$3").trim();
        } 
        else if (value.length === 8) {
            value = value.replace(/(\d{3})(\d{2})(\d{3})/, "$1-$2-$3").trim();
        }
        if (value.endsWith('-')) {
            value = value.slice(0, -1); // Remove the trailing hyphen if it's the last character
        }
        setCarNumber(value);
    }
    ...
    const getLineCus = (k) => {
        return <div onClick={() => { setProducer(k); setDisplaySearchPrdoucer('hidden'); }} className="hover:bg-primary-200 cursor-pointer p-1 bg-white m-2 z-30 rounded-lg">{k}</div>
    }
    ...
    useEffect(() => {
        const CheckIFThereCustomer = () => {
            if(customerSet){
                setCustomer(customerSet);
                setCustomerCheck(true);
                setShowPropsCarDriver(true);
                setShowPropsCustomer(false);
                setCarCheck(true);
                ...
                showForm ?
                    ...
                    <div className="shadow-2xl rounded-3xl bg-white p-5 mr-5 ml-5">
                        ...
                        </div>
                        {
                            ...
                            <>
                                <div className="w-full mt-10 mb-10">
                                    <div className="flex justify-around items-center mb-20">
                                        ...
                                        <div className="w-full flex justify-around items-center">
                                            <div className="ml-2 mr-2">
                                                <Button onClick={() => { setCustomer(null); resetAllCustomer(); setCustomerCheck(false); setCustomerShowInputs(false); setShowCustomerModdal(false); }} className="">
                                                    חדש <HiPlus />
                                                </Button>
                                            </div>
                                            <div className="ml-2 mr-2">
                                                <Button onClick={() => { setShowCustomerModdal(true); }} className="">
                                                    קיים <GoSearch />
                                                </Button>
                                            </div>
                                        </div>
                                    ...
                                    <div dir="ltr">
                                        {showCustomerModal && <ModalCustomer setCustomer={(cus) => { setCustomer(cus); setCustomerCheck(true);setShowPropsCarDriver(true);setShowPropsCustomer(false);}} show={showCustomerModal} disable={() => setShowCustomerModdal(false)} />}
                                    </div>
                                    {
                                        !showCustomerModal &&
                                        <div className={`max-[450px]:text-[13px]`}>
                                            <div className="flex items-center mt-5">
                                                <div className="w-[200px]">תמונה</div>
                                                <div className="w-full">
                                                    <input onChange={(e) => setcustomerImageDateFile(e.target.files[0])} ref={customerImageFile} className="hidden" type="file" />
                                                    <Button className={`text-${customerImageDateFile ? 'primary' : 'default'} border-${customerImageDateFile ? 'primary' : 'default'}`} onClick={() => { customerImageFile.current.click() }} variant="bordered" size="sm"><FaFolder /></Button>
                                                    {customerImageDateFile && <Button onClick={() => setcustomerImageDateFile(null)} color="danger" variant="light" className="">הסרה</Button>}
                                                </div>
                                            </div>
                                            <div className="flex items-center mt-5">
                                            ...
                                            <div className="flex items-center mt-5">
                                                <div className="w-[200px]">רישיון מוביל</div>
                                                <div className="flex items-center w-full">
                                                    <Input ref={reshionMovelRef} errorMessage={errorReshionMovel} isDisabled={customer ? true : false} value={customer ? customer.reshionMovel : reshionMovel} onValueChange={(value) => { setReshionMovel(value); CheckCustomerInputs(); setErrorReshionMovel(''); }} type="date" variant="bordered" size="sm" color="primary" className="w-full max-w-[300px] ml-3" />
                                                    <input onChange={(e) => setReshionMovelDateFile(e.target.files[0])} ref={reshionMovelFile} className="hidden" type="file" />
                                                    <Button className={`text-${reshionMovelDateFile ? 'primary' : 'default'} border-${reshionMovelDateFile ? 'primary' : 'default'}`} onClick={() => { reshionMovelFile.current.click() }} variant="bordered" size="sm"><FaFolder /></Button>
                                                    {reshionMovelDateFile && <Button onClick={() => setReshionMovelDateFile(null)} color="danger" variant="light" className="">הסרה</Button>}
                                                </div>
                                            </div>
                                            <div className="flex items-center mt-5">
                        ...
                        }
                        {
                            showPropsCarDriver &&
                            <>
                                {showModalSearchCarByNumber && <SearchByCarNumber show={showModalSearchCarByNumber} disable={() => setShowModalSearchByCarNumber(false)}/>}
                                <div className="w-full mt-10 mb-10">
                                    <div className="flex justify-around items-center mb-20">
                                        <div className="flex w-full justify-center items-center">
                                        ...
                                        <div className="w-full flex justify-around items-center">
                                            <div className="ml-2 mr-2">
                                                <Button onClick={() => { ResetAllFilesCar(); setCar(null); resetAllCar(); setCarCheck(false); }}>
                                                    חדש <HiPlus />
                                                </Button>
                                            </div>
                                            <div className="ml-2 mr-2">
                                                <Button onClick={() => { setShowCarModdal(true); }}>
                                                    קיים <GoSearch />
                                                </Button>
                                            </div>
                                            <div className="ml-2 mr-2">
                                                <Button onClick={() => { setShowModalSearchByCarNumber(true); }}>
                                                    מספר <TbAccessPoint className="text-xl"/>
                                                </Button>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="max-[450px]:text-[13px]">
                                        ...
                                        <div className="flex items-center mt-5">
                                            ...
                                            <div className="w-full flex items-center">
                                                <Input errorMessage={errorCarNumber} isDisabled={car ? true : false} value={car ? car.car_num : carNumber} onValueChange={(value) => { setCarNumber(value); sortCarNumber(value);CheckCarInputs(); setErrorCarNumber(''); }} type="text" variant="bordered" size="sm" color="primary" className="w-full max-w-[300px] ml-3" />
                                                <Button isDisabled={carCheck} onClick={GetVichel}>חיפוש רכב</Button>
                                            </div>
                                        </div>
                                        <div className="flex items-center mt-5">
                                            ...
                                            <div onClick={() => setDisplaySearchPrdoucer('')} className="w-full">
                                                <div className="relative max-w-[300px]">
                                                    ...
                                                    <div ref={SearchPrdoucerRef1} className="flex justify-end">
                                                        {
                                                            !car && ChoisesProducer() &&
                                                            <div className={`${displaySearchPrdoucer} absolute border border-gray-300 rounded-md bg-white w-full z-30 max-h-60 overflow-auto`}>
                                                                {
                                        ...
                                        <div className="flex items-center mt-5">
                                            ...
                                            <div className="w-full flex items-center">
                                                ...
                                                <Button isDisabled={carCheck} className={`text-${fileEndDateCar ? 'primary' : 'default'} border-${fileEndDateCar ? 'primary' : 'default'}`} onClick={() => { fileEndDateCarRef.current.click() }} variant="bordered" size="sm"><FaFolder /></Button>
                                        ...
                                        <div className="flex items-center mt-5">
                                            ...
                                            <div className="w-full flex items-center">
                                                ...
                                                <Button isDisabled={carCheck} className={`text-${fileInsurance ? 'primary' : 'default'} border-${fileInsurance ? 'primary' : 'default'}`} onClick={() => { fileInsuranceRef.current.click() }} variant="bordered" size="sm"><FaFolder /></Button>
                                                {fileInsurance && <Button onClick={() => setFileInsurance(null)} color="danger" variant="light" className="">הסרה</Button>}
                                            </div>
                                        </div>
                                        {
                                            ((car?.car_type === `רכב מעל 10,000 ק"ג`) || (typeCar === `רכב מעל 10,000 ק"ג`) || (typeCar === 'גרור')) &&
                                            <div className="flex items-center mt-5">
                                                ...
                                                <div className="w-full flex items-center">
                                                    ...
                                                    <Button isDisabled={carCheck} className={`text-${fileShockabsorbers ? 'primary' : 'default'} border-${fileShockabsorbers ? 'primary' : 'default'}`} onClick={() => { fileShockabsorbersRef.current.click() }} variant="bordered" size="sm"><FaFolder /></Button>
                                        ...
                                        <div className="flex items-center mt-5">
                                            ...
                                            <div className="w-full flex items-center">
                                                ...
                                                <Button isDisabled={carCheck} className={`text-${fileWinterreview ? 'primary' : 'default'} border-${fileWinterreview ? 'primary' : 'default'}`} onClick={() => { fileWinterreviewRef.current.click() }} variant="bordered" size="sm"><FaFolder /></Button>
                                                ...
                                                {errorTachograph && <div className="text-danger text-xs">{errorTachograph}</div>}
                                                {
                                                    ...
                                                    <div className="flex items-center mt-5">
                                                        ...
                                                        <div className="w-full flex items-center">
                                                            ...
                                                            <Button isDisabled={carCheck} className={`text-${fileTachograph ? 'primary' : 'default'} border-${fileTachograph ? 'primary' : 'default'}`} onClick={() => { fileTachographRef.current.click() }} variant="bordered" size="sm"><FaFolder /></Button>
                                        ...
                                        {errorHazmat && <div className="text-danger text-xs">{errorHazmat}</div>}
                                        {
                                            ...
                                            <div className="flex items-center mt-5">
                                                ...
                                                <div className="w-full flex items-center">
                                                    ...
                                                    <Button isDisabled={carCheck} className={`text-${fileHazmat ? 'primary' : 'default'} border-${fileHazmat ? 'primary' : 'default'}`} onClick={() => { fileHazmatRef.current.click() }} variant="bordered" size="sm"><FaFolder /></Button>
                                ...
                                <div className="w-full mt-10 mb-10">
                                    <div className="flex justify-around items-center mb-20">
                                        ...
                                        <div className="w-full flex justify-around items-center">
                                            <div className="ml-2 mr-2">
                                                <Button onClick={() => { ResetAllFilesDriver(); setDriver(null); resetAllDriver(); setDriverCheck(false);setIsWithDriver(true); }}>
                                                    חדש <HiPlus />
                                                </Button>
                                            </div>
                                            <div className="ml-2 mr-2">
                                                <Button onClick={() => { setShowDriverModdal(true);setIsWithDriver(true); }}>
                                                    קיים <GoSearch />
                                                </Button>
                                            </div>
                                            {
                                                isWithDriver ?
                                                    <div className="ml-2 mr-2">
                                                        <Button onClick={() => setIsWithDriver(false)}>
                                                            לדלג <IoPlaySkipBackOutline />
                                                        </Button>
                                                    </div>
                                                    :
                                                    <div className="ml-2 mr-2">
                                                        <Button isDisabled className="opacity-40" onClick={() => setIsWithDriver(false)}>
                                                            לדלג <IoPlaySkipBackOutline />
                                                        </Button>
                                                    </div>
                                ...
                                {
                                    staticErrorMessage &&
                                    <Card className="border-red-600 border-1 mt-8">
                                        <CardBody>
                                            <div className="flex items-center">
                                                <div>
